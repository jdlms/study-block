<script>
    ...
    async function onColorChange(e) {
        ...
        try {
            const res = await fetch(
                `/api/subjects/${encodeURIComponent(colorTarget)}`,
                {
                    method: "PATCH",
            ...
            subjects = subjects.map((subject) =>
                subject.name === updated.name ? updated : subject,
            );
        } catch (err) {
    ...
    function entriesForSubjectAndDate(subjectName, date) {
        ...
        return entries.filter(
            (entry) => entry.subject === subjectName && entry.date === dateStr,
        );
    }

    function totalMinutesForSubjectAndDate(subjectName, date) {
        return entriesForSubjectAndDate(subjectName, date).reduce(
    ...
    function openEditModal(date, preferredSubject = "") {
        ...
        editDate = date;
        editSubject =
            preferredSubject || selectedSubject || subjects[0]?.name || "";
        syncEditMinutes();
        editOpen = true;
    }
    ...
    async function saveStudyMinutes() {
        const nextMinutes = Number.parseInt(
            String(editMinutes ?? "").trim(),
            10,
        );
        ...
        try {
            ...
            await Promise.all(
                existing.map(async (entry) => {
                    ...
                    if (!res.ok) {
                        throw new Error(
                            (await res.text()) || "Failed to update entry",
                        );
                    }
                }),
            );
    ...
    function onChartClick(event, _elements, chart) {
        ...
        const preferredSubject = item
            ? filteredDatasets[item.datasetIndex]?.label || ""
            : "";
    ...
    $: datasets = subjects.map((subject) => ({
        ...
        borderWidth: dates.map((date) =>
            formatDate(date) === formatDate(today) ? 2 : 0,
        ),
        borderSkipped: false,
        borderRadius: 4,
...
<div class="shell">
    <div class="card">
        <div class="header">
            <h2>
                <button class="title-link" on:click={clearSubjectFilter}
                    >Study Blocks</button
                >
            </h2>
                ...
                {#each subjects as subject}
                    ...
                    >
                        ...
                        <span
                            ...
                            on:keydown={(e) => {
                                if (e.key === "Enter" || e.key === " ")
                                    toggleSubjectFilter(subject.name);
                            }}>{subject.name}</span
                        >
                    </div>
                {/each}
                ...
                >
                    ...
                    <div class="modal-actions">
                        <button type="button" on:click={closeEditModal}
                            >Cancel</button
                        >
                        <button type="button" on:click={saveStudyMinutes}
                            >Save</button
                        >
                    </div>
                </div>
